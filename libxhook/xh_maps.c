...
void xh_maps_update()
{
    ...
    while (fgets(line, sizeof(line), f_maps) != NULL)
    {
        ...
        if (pathname == NULL)
        {
            LOG_ERROR("Fail to allocate memory space to store pathname in maps item.");
            exit(-1);
        }
        ...
        if (s_xh_maps_maps_items.items == NULL || map_item_idx >= curr_map_item_count)
        {
            ...
            {
                LOG_ERROR("Fail to expand memory space to store maps items, exp_item_cnt: %zu, exp_size: %zu",
